-- IBES Detail History — Announce-with-Timestamp Excluded Estimates (adjusted)
-- File 11 in IBES Detail History User Guide (Dec 2016)
-- Source CSV: detail/excluded estimates.csv  (~48.9M rows)
...
DROP TABLE IF EXISTS lseg_ibes.excluded_estimates_adjusted;
CREATE TABLE lseg_ibes.excluded_estimates_adjusted (
    ...
    VALUE     Nullable(Float64)      COMMENT 'Analyst estimate value, split-adjusted basis',
    ...
    FPEDATS   Date32                 COMMENT 'Forecast Period End Date',
    ACTTIMS   String                 COMMENT 'Activation time',
    EXCDATS   Date32                 COMMENT 'Exclude date',
    EXCTIMS   String                 COMMENT 'Exclude time',
    EXCENDS   Date32                 COMMENT 'Exclude end date',
    EXCETIMS  String                 COMMENT 'Exclude end time'
) ENGINE = MergeTree
ORDER BY (TICKER, MEASURE, FPI, FPEDATS, ACTDATS, ESTIMATOR)
COMMENT 'IBES Detail History — Excluded Estimates (analyst-level), split-adjusted';

INSERT INTO lseg_ibes.excluded_estimates_adjusted
FROM INFILE 'excluded estimates.csv'
